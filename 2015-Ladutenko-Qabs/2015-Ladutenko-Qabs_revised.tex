...
these works the authors introduced the concept of superscattering, when
scattering cross-section of a multilayer particle exceeds that of
...
particle, resonances appear at different frequencies, and there is
...
(ACS) value for a single channel, i.e., when only one mode of a
...
the geometric cross-section of particles. Here we show that there
...
achieved in experimental realisations.
...
using experimentally measured parameters from Ref.~\cite{palik-1997}, and, e.g. at $\lambda = 500$ nm $\epsilon_{Si}
...
\begin{figure}
  \center{
    ...
    \caption{ Results of optimization of the absorption efficiency
      ...
      a two-layer structure, when core size vanishes, and the
...
spherical particles, here we introduce superabsorption regime,
when {\em the ACS is larger than the theoretical limit for absorption
  by the mode with the highest excited angular momentum $n$}. In our
parameter space we have just modes up to the quadrupole excited
($n=2$), and in order to get superabsorption our efficiency should
...
quadrupole. For small particles, as expected, absorption is
dominated by electric dipole $\tilde{a}_1$.  At $R > 56.6$~nm the
...
Fig.~\ref{fig:overview}~(c) shows optimized sizes of layers inside
a multilayer structure. It reveals quite a curious result, that the
dipole branch (i.e. for particle radii below 56.6~nm) has two
parts. For $R<46$~nm the best absorber has just two layers, as the
radius of the core of a three-layer structure vanishes, and the
...
volume with high absorption rate in the vortices does not compensate low
absorption efficiency in the regions with small electric field (absorbed power is expressed as $P_{\rm abs}=\frac{1}{2}\int\sigma
\left|E\right|^2dx\,dy\,dz$)

In conclusion, we introduce and study the effect of superabsorption, when the absorption cross-section of the
