...
the work of Estakhri and Al\`u~\cite{Alu-2014} and provide a number of
new designs. If we define the total amount of the absorbed power with
cross-section of $\alpha_{\rm abs}=\frac{3\lambda^2}{8\pi}$ our
optimization give a similar result for the structure with contributing
harmonics TM${_1}$ and TE$_1$.  For $\epsilon_1 = 1.29+i0.01$,
...
optimizer to keep high level of absoption efficiency $Q_{\rm abs}
\approx 5$ the particle becomes much smaller and the size of the outer
layer vanish $\{a_{c1},a_{c2}\}=\{0.00635,0.00747\}\lambda$ resulting
with $\eta_{\rm abs}^{(Q)} =544$.  This way small particle still
absorbes five times of its phisical size with a very large
scattering-normalized absorption efficiency.  Moreover, it is easy to
set optimizer to keep $\eta_{\rm abs} \approx 100$ or $\eta_{\rm abs}
\approx 1000$, resulting $\{a_{c1},a_{c2}\}=\{0.0112,0.0131\}\lambda$,
$Q_{\rm abs} = 8.65$ and $\{a_{c1},a_{c2}\}=\{0.0052,0.0061\}\lambda$,
